...
$stmt = $pdo->prepare('SELECT articles.*, users.full_name AS author FROM articles JOIN users ON articles.author_id = users.id 
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if (isset($_GET['is_published'])) {
        $new_published_status = ($_GET['is_published'] == 1) ? 0 : 1;
        $stmt = $pdo->prepare('UPDATE articles SET is_published = ? WHERE id = ?');
        $stmt->execute([$new_published_status, $article_id]);
        $_SESSION['messages'][] = "Article publish status has been updated.";
...
// Step 8: If the 'is_featured' control is clicked, toggle the status from 0 -> 1 for featured or 1 -> 0 for unfeatured
    if (isset($_GET['is_featured'])) {
        $new_featured_status = ($_GET['is_featured'] == 1) ? 0 : 1;
        $stmt = $pdo->prepare('UPDATE articles SET is_featured = ? WHERE id = ?');
        $stmt->execute([$new_featured_status, $article_id]);
        $_SESSION['messages'][] = "Article featured status has been updated.";
        header('Location: articles.php');
        exit;
    }
}
